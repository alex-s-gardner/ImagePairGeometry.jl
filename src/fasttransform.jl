...
"""
    fast_transform(grid_crs, image_crs; always_xy = false) -> TransformPair
...
result as `pairgeometry`'s or `pairgeometry_blocked`'s `transform`; unlike the PROJ path it needs no
factory, because the transformation holds no per-thread state.
...
```
"""
function fast_transform(grid_crs, image_crs; always_xy::Bool = false)
